...
//6、支持多反代，支持外部环境变量，变量名【PROXYIP】，每行一个，无限尝试逻辑（WORKER响应都是毫秒级的，几乎感觉不到反代切换的影响），直到连上了或者所有反代都无效，优先次序是【外部环境变量定义的PROXYIP--脚本内部定义的反代IP】
...
const 哎呀呀这是我的ID啊 = "66666"; //实际上这是你的订阅路径，支持任意大小写字母和数字，[域名/ID]进入订阅页面
...
const 咦这是我的私钥哎 = ""; //这是你的私钥，提高隐秘性安全性，就算别人扫到你的域名也无法链接，再也不怕别人薅请求数了^_^
...
let 我的优选IPV6 = 'www.visa.com' //CF的IPV6节点，这个只增加了clash订阅，填域名或IP，好的优选一个就够了，由于CFcdn常规13端口开放，可以生成全端口节点
...
let 反代IP = [
  ...
  'ts.hpc.tw',
] //反代IP或域名，支持多反代，每行一个，不需要填端口，反代IP只是兜底策略，不能固定落地地区，可以结合非CF节点一起用并选择该节点，固定落地地区
...
const 伪装网页 = 'https://cfip.nyc.mn/' //填入伪装网页，格式'www.youku.com'，如果不填，脚本本身有个内置的简单代理页面，建议用小站伪装或者直接内置，比较靠谱
...
async function 解析VL标头(VL数据) {
    if (验证UUID(new Uint8Array(VL数据.slice(1, 17))) !== 哎呀呀这是我的UUID) {
		return null;
	  }
...
function 验证UUID(arr, offset = 0) {
	const uuid = 提取UUID(arr, offset);
	return uuid;
}
function 提取UUID(arr, offset = 0) {
...
async function TCP握手协议(远程传输, 识别地址类型, 访问地址, 访问端口, 写入数据请求, WS接口) {
  ...
  async function 原始反代兜底() {
    const 反代TCP接口 = await 创建TCP握手(反代IP库, 访问端口);
...
let FDIP, 反代IP库, 反代IP库计数器 = 0, 反代数组;
function 获取反代IP列表() {
  if (FDIP) {
...
async function 创建SOCKS5接口(识别地址类型, 访问地址, 访问端口) {
	const { username, password, hostname, port } = 获取SOCKS5账号(我的SOCKS5账号);
  const SOCKS5接口 = await connect({ hostname, port });
  ...
  await writer.write(socksGreeting);
	...
	reader.releaseLock();
	return SOCKS5接口;
  function 关闭接口并退出(){ //关闭SOCKS5接口，重新创建原始接口防止卡死
    if (writer) {
      writer.releaseLock();
    }
    if (reader) {
...
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:80?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:8080?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:8880?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:2052?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:2082?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:2086?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:2095?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:443?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:8443?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:2053?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:2083?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:2087?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选}:2096?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:80?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:8080?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:8880?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:2052?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:2082?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:2086?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:2095?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:443?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:8443?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:2053?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:2083?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:2087?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${我的优选IPV6}:2096?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${特殊优选}:${特殊优选的端口}?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
${转码}${转码2}${符号}${哎呀呀这是我的UUID}@${hostName}:443?encryption=none&security=tls&type=ws&host=${hostName}&path=%2F%3Fed%3D2560
`);
if (私钥开关) {
  return `请先关闭私钥功能`
}else {
